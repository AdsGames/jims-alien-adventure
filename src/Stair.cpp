...
void Stair::load_sprites() {
  ...
  images[IMG_STAIRS] =
      load_png_ex(("images/levels/" + folder + "/stairs.png").c_str());
  images[IMG_TOP_RED] =
      load_png_ex(("images/levels/" + folder + "/stage_end_red.png").c_str());
  images[IMG_TOP_GREEN] =
      load_png_ex(("images/levels/" + folder + "/stage_end_green.png").c_str());
  images[IMG_BRICK] =
      load_png_ex(("images/levels/" + folder + "/brick.png").c_str());
}
...
void Stair::draw(BITMAP* buffer) {
  ...
  for (int i = x + images[IMG_STAIRS]->w - 30; i < SCREEN_W;
       i += images[IMG_BRICK]->w) {
  ...
  if (type == 1 || type == 2) {
    blit(transparency_bitmap, buffer, 0, 0, x + images[type]->w, y, SCREEN_W,
         SCREEN_H);
    line(buffer, x + images[type]->w - 1, y + 30, x + images[type]->w - 1,
         SCREEN_H, makecol(168, 148, 148));
  }
}
